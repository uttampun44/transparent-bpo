        ...
        <div class="ml-[200px]">
            <div class= "overflow-hidden shadow-sm sm:rounded-lg md:px-8 md:py-8 bg-white">
                <div class="form-container">
                    <form method="POST" action="{{route('create.jobs', $jobs->id)}}" enctype="multipart/form-data">
                       @csrf
                       @method('POST')
                        <div class="job-title my-4 max-md:text-center">
                            <label class="text-black text-2xl font-medium leading-normal font-sans" for="postjobs">Update Jobs :</label>
                        </div>
                        <div class="job-category my-4 pr-4">
                            <select name="jobcategory" class="w-[50%] rounded-md">
                                @foreach ($category as $item)
                                     <option value="{{$item->id}}"  {{ $item->id == $jobs->job_category_id ? 'selected' : '' }}>{{$item->job_categories}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="job-grid grid grid-cols-2 gap-x-5 gap-y-6 max-md:flex max-md:flex-col max-md:px-4">
                            ...
                            <div class="company-name">
                                ...
                                <input type="text" name="companyname" class="py-2 rounded-md w-[100%] mt-4 text-lg font-medium leading-normal font-sans" value="{{$jobs->company_name}}" />
                           ...
                           <div class="job-post-cols">
                            ...
                            <input type="text" name="jobpost" class="rounded-md w-[100%] mt-4 text-lg font-medium leading-normal font-sans" value="{{$jobs->job_post}}"/>
